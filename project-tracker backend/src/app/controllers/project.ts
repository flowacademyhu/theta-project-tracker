...
export const index = async (req: Request, res: Response) => {
  let query: QueryBuilder = database(TableNames.projects).select().where({deletedAt: null});
  if (req.query.limit) {
...
export const show = async (req: Request, res: Response) => {
  try {
    const project: Project = await database(TableNames.projects).select().where({ id: req.params.id, deletedAt: null }).first();
    if (project) {
      res.json(project);
    } else {
      res.sendStatus(404);
